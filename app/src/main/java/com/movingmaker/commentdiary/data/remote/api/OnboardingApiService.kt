...
interface OnboardingApiService {
    ...
    @GET(SEND_EMAIL_CODE)
    suspend fun sendEmailCode(@Query("email") email: String): Response<BaseResponse<Nothing>>

    @POST(EMAIL_CODE_CHECK)
    suspend fun emailCodeCheck(@Body emailCodeCheckRequest: EmailCodeCheckRequest): Response<BaseResponse<Nothing>>

    @POST(SIGNUP)
    suspend fun signUp(@Body signUpRequest: SignUpRequest): Response<BaseResponse<Nothing>>
    ...
    @GET(FIND_PW)
    suspend fun findPassword(@Query("email") email: String): Response<BaseResponse<Nothing>>
}